...
for arquivo in arquivos:
    ...
    # if arquivo.endswith(".WAV"):
        ...
        if ((df["ID_ONDA_CAMPO"] == ID) & (df["NET"] == "NS/NR")).any():
            count_NSNR += 1
            NS_NR.append(ID)
        elif ((df["ID_ONDA_CAMPO"] == ID) & (df["NET"] == 'Áudio não encontrado')).any():
            count_audio_nao_encontrado += 1
            Audio_nao_encontrado.append(ID)
        elif ((df["ID_ONDA_CAMPO"] == ID) & (df["Credenciadora"] != Credenciadora)).any():
